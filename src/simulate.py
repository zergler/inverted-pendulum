...
class Simulation(object):
    ...
    def __init__(self, agent, environment):
        ...
        lines = plt.plot(self.e.t, self.e.state[:, :self.e.state.shape[1] / 2])
        lab = plt.xlabel('Time [sec]')
        leg = plt.legend(dynamic[:self.e.state.shape[1] / 2])
        animate_pendulum(self.e.t, self.e.state, self.e.arm_length, filename="../data/test.mp4")
...
def main():
    ...
    t_sim = 5
    t_resol = 100
    e = environment.Environment(links, arm_length, arm_mass, cart_mass, cart_length, rail_length, gravity, t_sim, t_resol)
    s = Simulation(None, e)
